...
static const int tagButtonBase = 100; // HYSelectedButton tag 标签基数
static const int tagTableViewBase = 200; // UITableView tag 标签基数
static const int TagCellLabel = 10; //Label 标签
...
@property (nonatomic, strong) NSArray *titles; //titles显示数组,有序的
...
- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titles dataDict:(NSMutableDictionary *)dict{
    ...
    if (self) {
        ...
        for (int i = 0; i < self.titles.count; i++) {
            ...
            tableView.tag = i + tagTableViewBase;
...
- (void)layoutSubviews{
    ...
    for (int i = 0; i < self.titles.count; i++) {
        HYSelectedButton *button = [self viewWithTag:i + tagButtonBase];
        button.frame = CGRectMake(i * width, 0, width, self.toolBar.frame.size.height);
        
        UITableView *tableView = [self viewWithTag:i + tagTableViewBase];
...
- (void)hySelectedButton:(HYSelectedButton *)hyButton{
    for (int i = 0; i < self.titles.count; i++) {
        ...
        NSInteger selectedBtnTag = i + tagButtonBase;
        ...
        NSInteger tableTag = i + tagTableViewBase;
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    NSString *key = self.titles[tableView.tag - tagTableViewBase];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UILabel *label = [cell.contentView viewWithTag:TagCellLabel];
    label.text = value[indexPath.row];
    label.textColor = self.selIndexPath.row == indexPath.row ? self.selectedTitleColor : self.normalTitleColor;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSInteger num = tableView.tag - tagTableViewBase; //知道点击了哪个tableview以及对应的 num 值
    ...
    for (int i = 0; i < number; i++) {
        ...
        UILabel *label = [cell.contentView viewWithTag:TagCellLabel]; //取出每一行的cell中的label控件
    ...
    HYSelectedButton *button = [self viewWithTag:num + tagButtonBase]; //把选中的tableview对应的HYSelectedButton设置为未选中状态
    button.normalTitle = value[indexPath.row]; //把筛选title变更为选中的文字
    button.selectedTitle = value[indexPath.row]; //把筛选title变更为选中的文字
    button.selected = NO;
...
- (void)refreshLayout:(HYSelectedButton *)hyButton{
    ...
    rect.size.height = hyButton.selected ? self.allHeight : self.toolBarHeight;
    self.frame = rect;
}
